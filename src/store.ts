...
type GameState = {
  profile: Profile; visited: string[]; scores: Record<string, number>; answers: Record<string, string[]>; resultSubmitted: boolean;
  setAvatar: (avatar:number)=>void; setProfile:(profile:Partial<Profile>)=>void;
  answer:(place:string, values:string[], scoreKey:string)=>void; markResultSubmitted:()=>void; reset:()=>void;
};
const initial = { profile:{avatar:-1,year:'',program:'',accessCode:'',sid:''}, visited:[], scores:{R:0,I:0,A:0,S:0,E:0,C:0}, answers:{}, resultSubmitted:false };
export const useGame = create<GameState>()(persist((set)=>({
  ...
  markResultSubmitted:()=>set({resultSubmitted:true}),
  reset:()=>set(initial)
