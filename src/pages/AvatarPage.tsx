...
export default function AvatarPage() {
  ...
  return <GameShell cyan><Topbar/><section className="panel narrow avatar-panel">
    ...
    <div className="avatar-grid">{avatars.map((avatar,index)=>{
      ...
      return <motion.button aria-label={`Choose avatar ${index+1}`} key={avatar} className={cn(selected===index&&'selected')} onClick={()=>setAvatar(index)} whileTap={{scale:.94}}><img className={`avatar-art avatar-art-${index}`} src={asset(RESOURCE_ROOT+'avatar icon/'+image)} alt=""/></motion.button>;
    })}</div>
