...
import { isResultSubmissionBypassed, resultApiEndpoint } from '../config/runtime';
...
export default function ResultsPage(){
  ...
  useEffect(()=>{
    ...
    let cancelled=false;
    fetch(resultApiEndpoint,{
      method:'POST',
      headers:{'Content-Type':'application/json',Accept:'application/json'},
      body:JSON.stringify({sid:profile.sid,year:profile.year,program:profile.program,accessCode:profile.accessCode,avatar:profile.avatar,answers})
    }).then(response=>response.json().catch(()=>({ok:false}))).then(payload=>{
      if(cancelled)return;
      if(payload?.ok)markResultSubmitted();
    }).catch(()=>undefined);
